  ...
  if(blacklist.get(`${message.guild.id}-${message.author.id}`) || blacklist.get(`${message.guild.id}-${message.channel.id}`)) return;
      ...
      // level up!
        ...
        let embed = new Discord.MessageEmbed()
              ...
              if(!customSettings)
              {
                embed.setDescription(`**Félicitations !** ${message.author}! Vous avez désormais atteint le niveau **level ${level.level}**`);
                levelUpMsg = `**Félicitations !** ${message.author}! Vous avez désormais atteint le niveau **level ${level.level}**`;
              } else {
